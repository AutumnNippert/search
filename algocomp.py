...
if len(sys.argv) <= 3:
    print("Usage: python3 korfcomp.py <path to executable> <path to folder> [algos...]")
    exit(1)
...
for file in lst: # non inclusive
    ...
    for algo in algo_list:
        ...
        if algo not in cumulative_times:
            cumulative_times[algo] = 0
        cumulative_times[algo] += cpu_time
        if algo not in algo_json_results:
            algo_json_results[algo] = []
        algo_json_results[algo].append(AlgoResult.from_output(file, output)) # this * thing is kinda cool
        f.close()
